...
public class InsertArchivos {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < paths.length; i++) {
			...
			System.out.println(
					"INSERT INTO `Sucursal`.`Archivos` ('Contenido', `mimeType`, `Tamaño`) VALUES ((LOAD_FILE('" + path
							+ "')), '" + mimeType + "', '" + kb + " ');");
		...
		for (int i = 0; i < idsProductos.length; i++) {
			...
			for (int j = 0; j < cantIm; j++) {
				System.out.println("INSERT INTO `Sucursal`.`Productos_has_Archivos` (`idProductos`, `idArchivos`) VALUES ('" + idsProductos[i] + "', '"+ j +"');");
			}
		}
	}
}
